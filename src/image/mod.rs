...
pub trait IsImageDecoder {
    type MyType;
    fn decode<T: IsImageData>(image: &T) -> Result<Self::MyType, std::io::Error>;
}

pub trait Loadable {
    type MyType;
    fn load(filename: String) -> Result<Self::MyType, io::Error>;
}
...
impl Loadable for RawImageData {
    ...
    fn load(filename: String) -> Result<RawImageData, io::Error> {
